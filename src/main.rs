...
struct App {
    ...
    // Browser tab
    items: Vec<FolderInfo>,      // Current items (files and folders) - name kept as FolderInfo for compatibility
    all_items: Vec<FolderInfo>,   // Unfiltered list of all items
...
impl App {
    fn new(remote_source: String, local_dest: String) -> io::Result<Self> {
        ...
        let mut items = list_remote_folders(&remote_host, &current_path, &ssh_control_socket)?;
    ...
    fn apply_filter(&mut self) {
        ...
        if self.filter_query.is_empty() {
            ...
            let mut scored_items: Vec<(i64, FolderInfo)> = self.all_items
                .iter()
                .filter_map(|item| {
                    matcher.fuzzy_match(&item.name, &self.filter_query)
                        .map(|score| (score, item.clone()))
                })
                .collect();

            scored_items.sort_by(|a, b| b.0.cmp(&a.0));
            self.items = scored_items.into_iter().map(|(_, f)| f).collect();
        }

        // Reset selection
        self.browser_list_state.select(if self.items.is_empty() { None } else { Some(0) });
    }
    ...
    fn confirm_filter(&mut self) {
        ...
        let msg = if self.filter_query.is_empty() {
            ...
            format!("Filter: {} ({} results)", self.filter_query, self.items.len())
        };
    ...
    fn cancel_filter(&mut self) {
        ...
        let msg = if self.filter_query.is_empty() {
            ...
            format!("Filter restored: {} ({} results)", self.filter_query, self.items.len())
        };
        self.status_message = msg;
    }
    ...
    fn page_up(&mut self) {
        let (list_state, len) = match self.current_tab {
            Tab::Browser => (&mut self.browser_list_state, self.items.len()),
    ...
    fn enter_folder(&mut self) -> io::Result<()> {
        ...
        if let Some(i) = self.browser_list_state.selected() {
            ...
            // Only allow entering directories
            if !item.is_directory {
                self.status_message = format!("Cannot enter file: {}", item.name);
                return Ok(());
            }

            let folder = item.name.clone();
    ...
    fn queue_download(&mut self) {
        ...
        if let Some(i) = self.browser_list_state.selected() {
            let item_name = &self.items[i].name;
            ...
            self.status_message = format!("Queued: {}", item_name);
...
fn list_remote_folders(remote_host: &str, remote_path: &str, control_socket: &str) -> io::Result<Vec<FolderInfo>> {
    ...
    // List files and folders using SSH ControlMaster for connection reuse
    // Use find with -printf to get type information (d=directory, f=file, l=symlink)
    // Explicitly include files with -type f and directories with -type d (using -o for OR)
    let output = Command::new("ssh")
        ...
        .arg(remote_host)
        .arg(format!(
            "find {} -maxdepth 1 \\( -type f -o -type d \\) -not -path {} -printf '%p\\t%y\\n'",
            path, path
        ))
        .output()?;
    ...
    let folders: Vec<FolderInfo> = String::from_utf8_lossy(&output.stdout)
        ...
        .filter_map(|line| {
            ...
            // Split by tab to get path and type
            ...
            Some(FolderInfo { name, is_directory })
        })
        .collect();
